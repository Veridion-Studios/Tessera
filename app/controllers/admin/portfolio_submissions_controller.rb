module Admin
  class PortfolioSubmissionsController < BaseController
    ...
    def approve
      ...
      PortfolioApprovedNotification
        .with(title: @submission.title)
        .deliver(@submission.user)
    ...
    def reject
      @submission = PortfolioSubmission.find(params[:id])
      @submission.update!(status: "rejected", admin_notes: params[:admin_notes])

      PortfolioRejectedNotification
        .with(title: @submission.title, reason: params[:admin_notes])
        .deliver(@submission.user)
